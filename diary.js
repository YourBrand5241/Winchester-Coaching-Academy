...
const OWNER_PASSWORD = "winchesterowner2026";

// Same two schedules as the booking page: coaching on weekdays,
...
function getScheduleLabelForDate(dateStr) {
  const day = new Date(dateStr + "T00:00:00").getDay();
  if (SCHEDULES.coaching.days.includes(day)) return "Coaching sessions";
  if (SCHEDULES.party.days.includes(day)) return "Birthday parties";
  return "";
}
...
async function loadDiary() {
  ...
  if (!schedule) {
    listEl.innerHTML = "";
    messageEl.textContent = "Closed on that day.";
    return;
  }

  messageEl.textContent = `Loading — ${getScheduleLabelForDate(date)} day…`;
  listEl.innerHTML = "";
  ...
  messageEl.textContent = getScheduleLabelForDate(date);
